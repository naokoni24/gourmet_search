...
- 検索結果上部の「検索条件」タグ一覧（`src/app/page.tsx`の`getSearchConditionTags`）は、現在地検索の場合だけ他の条件（キーワード・ジャンル・距離等、オレンジの薄い丸タグ）と区別して緑背景＋`MapPin`アイコン付きの目立つタグ（「現在地」）で表示する。地名指定時は従来通り「場所: ○○」のオレンジタグのまま。

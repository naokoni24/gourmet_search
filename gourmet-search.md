...
- 【2026-07-05 修正済み】以前はジャンルを絞り込みモーダルで選択してもGoogle検索クエリの文字列に混ぜるだけで、結果を厳密にそのジャンルへ絞り込んではいなかった（例：ジャンル「焼肉」で検索すると焼肉屋以外の店舗も混ざる、ユーザー報告で発覚）。`backend/app/api/search.py`の`filter_list`に`if genre: items = [r for r in items if any(genre in g for g in r.genre)]`を追加し、Googleが返す実際の`genre`（`primaryType`/`primaryTypeDisplayName`由来、例:「焼肉店」「韓国焼肉店」）に選択ジャンルが部分一致する店舗だけに絞るよう修正。部分一致にしているのは、TYPE_MAPに無い`primaryType`（例:barbecue_restaurant）はGoogleの生の表示名（「焼肉店」等）がそのまま`genre`に入り、UIの選択肢（「焼肉」）と完全一致しないため。ローカルで焼肉・ラーメン・和食・居酒屋・イタリアン・韓国料理を確認し、いずれも選択ジャンルに一致する店舗のみが返ることを確認済み。

...
### 検索結果の復元（`src/app/page.tsx`）

- 【2026-07-05 追加】「店舗詳細を見る」「口コミを見る」はいずれも外部サイトへ`target="_blank"`で新規タブ遷移するため、理屈上は元のタブの検索結果は消えないはずだが、モバイルではタブ切り替えUIが分かりにくく「検索結果画面に戻りにくい」というユーザー報告があった。ブラウザの仕様上、外部サイト側から元のタブへ強制的にフォーカスを戻す手段は無い（`window.opener`はセキュリティ上`rel="noopener"`で無効化済みで、そもそもJSから他タブをアクティブにする操作自体がブラウザに存在しない）ため、タブ切り替えを支援する代わりに「検索画面に戻った時に結果が必ず残っている」方式にした。
- 直近の検索1回分（`params`・`searchedParams`・`restaurants`）を`sessionStorage`のキー`gourmet-search:last-search`に保存し、`page.tsx`マウント時（`useEffect`）に読み込んで自動復元する。検索成功時に毎回上書き保存し、エラー時（バックエンド接続失敗）は上書きしない（直前の正常な結果を保持するため）。
- これにより、タブ切り替えで元タブに戻る場合はもちろん、ブラウザの「戻る」操作やiOSの背景タブ再読み込みなど何らかの理由でページがリロードされた場合でも、直前の検索結果がAPIを叩き直さずに即復元される。ローカルで実際にページをリロードし、バックエンドに追加リクエストが飛ばずに55件の検索結果がそのまま復元されることを確認済み。
- スクロール位置の復元は対象外（ブラウザ標準のスクロール位置復元に委ねる）。
...
- 【2026-07-05 追加】場所（駅名・エリア入力 または 現在地）を指定していない状態でジャンル・キーワードだけで検索すると、Google側が現在地情報なしの全国区の関連度ランキングで返してくるため、結果が期待と大きくずれることがある。当初は検索結果表示後に注意バナーを出す案を検討したが、ユーザーの要望で「そもそも場所未指定なら検索自体をさせない」方針に変更。`SearchBar`の「検索」ボタンと`FilterModal`の「適用して検索」ボタンの両方に`hasLocation = params.place.trim() !== '' || params.current_lat != null`という共通ロジックを実装し、`false`の間は両ボタンを`disabled`にして「駅名・エリアを入力するか、現在地を使うと検索できます」という案内テキストを表示する。ジャンル・キーワードだけを先に設定してから場所を後入力する操作は引き続き可能（ボタンが自動的に有効化される）。
...
- 【2026-07-05 修正済み】以前はジャンルを絞り込みモーダルで選択してもGoogle検索クエリの文字列に混ぜるだけで、結果を厳密にそのジャンルへ絞り込んではいなかった（例：ジャンル「焼肉」で検索すると焼肉屋以外の店舗も混ざる、ユーザー報告で発覚）。`backend/app/api/search.py`の`filter_list`に、Googleが返す実際の`genre`（`primaryType`/`primaryTypeDisplayName`由来、例:「焼肉店」「韓国焼肉店」）に選択ジャンルが部分一致する店舗だけに絞る処理を追加。部分一致にしているのは、TYPE_MAPに無い`primaryType`（例:barbecue_restaurant）はGoogleの生の表示名（「焼肉店」等）がそのまま`genre`に入り、UIの選択肢（「焼肉」）と完全一致しないため。
  ...
  - 【2026-07-05 対応見送り・既知の制約として記録】本番で「キーワード：焼きとん」＋「荻窪」検索時に、Google側で`genre: ["焼き鳥"]`に分類される「串焼〇荻」という店が混ざるとの報告。ローカルで再現（半径2000mで再現、500mでは再現せず）。原因はGENRE_LABELSによる完全一致フィルタと根本的に性質が異なり、(1) 「焼きとん」はジャンルの選択肢に無い自由入力キーワードであり、(2) Googleの店舗分類自体に「焼きとん」という区分が存在せず、焼きとん屋も焼き鳥屋も同じ「居酒屋」等の大枠に分類されがちで、(3) 店名ベースで絞ろうとしても「やきとん」（ひらがな）と「焼きとん」（漢字）のような表記ゆれがあり単純な文字列一致では正規の焼きとん屋まで取りこぼす、という三重苦のため、GENRE_LABELS方式の横展開では直せない。ユーザーに対応方針（現状維持／専用の同義語フィルタを追加／キーワードの部分一致で緩く絞る）を確認したところ「現状維持（修正しない）」との回答だったため、コード変更はせず本項目に制約として記録するのみとした。自由入力キーワードでの検索は、Google Text Searchの関連度ランキングに依存する以上、ジャンル選択ほどの厳密さは保証されない仕様として扱う。
  - 【2026-07-05 修正済み】「荻窪＋ジャンル『洋食』＋500m」の結果が2件しか出ないとの報告を調査。原因はGENRE_LABELS完全一致フィルタの副作用で、Googleの`primaryType`は洋食系だけでも`western_restaurant`(洋食)・`bistro`(ビストロ)・`steak_house`(ステーキ)・`french_restaurant`(フレンチ)・`american_restaurant`(アメリカン)・`hamburger_restaurant`(ハンバーガー)・`restaurant`(レストラン)のように細分化されているため、生60件のうち文字通り「洋食」表記の2件以外が全部弾かれていた。UIの「ジャンル」ドロップダウンは大まかな括りとして機能することを期待されているのに対し、TYPE_MAPは表示用に細かく分けているというギャップが原因。`GENRE_GROUPS`（`search.py`）を新設し、ジャンルごとに許容する実際のGoogle表記の集合を定義（洋食→{洋食,ビストロ,ステーキ,フレンチ,アメリカン,ハンバーガー,ピザ}、和食→{和食,とんかつ,天ぷら,しゃぶしゃぶ,すき焼き,沖縄料理}、居酒屋→{居酒屋,焼き鳥}、イタリアン→{イタリアン,ピザ}、未定義のジャンルは従来通り自分自身のみ）。`_genre_matches`関数でこの許容集合のいずれかが店舗のgenreに部分一致すればOKとする方式に変更。ローカルで洋食が2→9件に改善し、焼肉・和食・居酒屋など他ジャンルも意図通り機能することを確認済み。
  - 【2026-07-05 追加修正】上記の初回対応で「洋食」に含めていた汎用ラベル「レストラン」が原因で、沖縄料理店（Googleが具体的な区分を持たず`restaurant`→「レストラン」に分類することがある）が「洋食」検索に混入するとの報告。「レストラン」はどの料理ジャンルにも属しうる汎用ラベルで特定の料理と結びつけられないため、GENRE_GROUPSのどのグループからも除外した（洋食から削除）。あわせて「和食」で検索した際に沖縄料理店を確実に拾えるようユーザーに確認し、既存の「焼肉」補完クエリ（エリア検索時に`{place} 焼肉`を追加でGoogleに投げてマージする仕組み）と同様の方式を追加する方針で合意。ジャンル・キーワードいずれかが「和食」の場合、メインクエリに加えて`{place} 沖縄料理`をText Search（`AREA_SUPPLEMENT_COUNT`件）で補完取得し`_merge_unique`でマージするようにした。ローカルで「荻窪＋和食＋2000m」が15件→18件に増え、沖縄料理店2軒（沖縄ゆいま〜る酒場 くいなくいな／なんちち、いずれもgenre:和食）が含まれるようになったこと、「洋食」「焼肉」など他ジャンルに影響が無いことを確認済み。
  - 【2026-07-05 追加修正】「荻窪 洋食ツバキ亭」という店名に「洋食」を含む店が、Googleの分類上は`japanese_restaurant`→「和食」になっており、洋食検索に出てこない・和食検索に出てしまうとの指摘。Googleの`primaryType`分類は店名の表記と食い違うことがある仕様（同名の店でGoogle側のAI分類が実際の業態と異なるのは珍しくない）だが、店名に明確なジャンル名が入っている場合はそちらを信頼する方が実用上正確なため、`_apply_name_genre_override`関数を追加。`filter_list`の先頭で全結果に対して実行し、`GENRE_LABELS`（10種）のいずれかが店名に含まれていれば、Googleのgenre分類を無視してそのラベル1つに上書きする（例:「洋食 ツバキ亭 レストラン 荻窪」→`genre: ['洋食']`に上書き）。この上書きは検索フィルタ・カード上のジャンルバッジ表示・画像選択（`RestaurantCard.tsx`の`getGenreVisual`）すべてに反映される。ローカルで「洋食ツバキ亭」が洋食検索に出るようになり、和食検索からは消えることを確認済み（他ジャンルの結果件数に変化なし）。なお`GENRE_VISUALS`（`RestaurantCard.tsx`）には「洋食」専用の画像バケットが無いため、上書き後は既定の`restaurant.png`（ラベル「レストラン」）にフォールバックする（専用画像を用意する場合は別途対応が必要）。
